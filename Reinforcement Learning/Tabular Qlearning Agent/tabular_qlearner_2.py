...
class QLearner(rl_agent.AbstractAgent):
    ...
    def _epsilon_greedy(self, info_state, legal_actions, epsilon):
        ...
        for legal_action in legal_actions:
          q_value = self._q_values[info_state][legal_action]
          probs[legal_action] = epsilon / len(legal_actions)
          if q_value > best_q_value:
            best_q_value = q_value
        best_actions = []
        for legal_action in legal_actions:
          if self._q_values[info_state][legal_action] == best_q_value:
            best_actions.append(legal_action)
        for best_action in best_actions:
          probs[best_action] += (1 - epsilon) / len(best_actions)
        ...
        if self._prev_info_state and not is_evaluation:
            ...
            if not time_step.last():
              ...
              for legal_action in legal_actions:
                ...
                if q_value > best_q_value:
                  best_q_value = q_value
              reward += self._discount_factor * best_q_value
              
            board = info_state_to_board(time_step)
            if self._player_id == 1 and board[0][1] + board[1][0] + board[1][2] == 3:
              reward = 1000
            if self._player_id == 0 and board[0][1] + board[1][0] + board[1][2] == -3:
              reward = 1000

            prev_q_value = self._q_values[self._prev_info_state][self._prev_action]
            self._last_loss_value = reward - prev_q_value
